    ...
    for (String sentence : sentences) {
      ...
      for (int i = 0; i < wordsInSentence.length; i++) {
        ...
        if (!words.contains(currentWord))
          continue; // Skip words not in the focused set

        int start = Math.max(0, i - windowSize);
        int end = Math.min(wordsInSentence.length, i + windowSize + 1);
        for (int j = start; j < end; j++) {
          if (j != i) { // Don't count the word itself
            String neighborWord = wordsInSentence[j];
            if (words.contains(neighborWord)) {
              HashCounter<String> counter = coMat.get(currentWord);
              counter.increment(neighborWord);
              // Ensure symmetry
              HashCounter<String> neighborCounter = coMat.get(neighborWord);
              neighborCounter.increment(currentWord);
            }
          }
        }
      }
